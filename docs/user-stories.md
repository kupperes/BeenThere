...
### US-2.1 [ ] As a developer, I can import one national historical dataset into the normalized place model.
Acceptance criteria:
...
### US-2.2 [ ] As a developer, I can import one state or local historical dataset into the same model.
Acceptance criteria:
- A second importer exists for a state or local source.
- Both importers normalize into the same `HistoricSite` schema.

### US-2.3 [ ] As a developer, I can retain source attribution for every imported record.
Acceptance criteria:
...
### US-2.4 [ ] As a developer, I can attach summary text and reference links, including Wikipedia when available.
Acceptance criteria:
...
### US-2.5 [ ] As a developer, I can track import runs and failures for debugging and repeatability.
Acceptance criteria:
...
### US-2.6 [ ] As a user, I can trust that places shown in the app reference a real source.
Acceptance criteria:
...
### US-3.1 [ ] As a user, I can open the app and allow location access.
Acceptance criteria:
...
### US-6.1 [ ] As an iPhone user, I can see nearby historical places on a mobile map.
Acceptance criteria:
...
### US-6.2 [ ] As an iPhone user, I can open a place detail card with summary, sources, and visited state.
Acceptance criteria:
...
### US-6.3 [ ] As an iPhone user, I can mark a place as visited from the detail card.
Acceptance criteria:
...
### US-6.4 [ ] As an iPhone user, I can review my collection and badges in a mobile-friendly view.
Acceptance criteria:
- The iPhone app exposes a collection or profile screen.
- The view includes visit totals and earned badges.

### US-6.5 [ ] As an iPhone user, I can use the app comfortably without excessive battery drain.
Acceptance criteria:
...
### US-6.6 [ ] As an iPhone developer, I can consume stable JSON endpoints in Xcode without relying on Django template behavior.
Acceptance criteria:
...
### US-6.7 [ ] As an iPhone developer, I can map backend responses directly into Swift models with minimal transformation.
Acceptance criteria:
- API fields use stable names and consistent types.
- Response shapes are simple enough to decode into Swift `Codable` models.

### US-6.8 [ ] As a product team, we can make backend decisions that prioritize iPhone usage instead of desktop compatibility.
Acceptance criteria:
...
### US-S1 [ ] As a user, my account credentials are handled securely.
Acceptance criteria:
...
### US-S2 [ ] As a user, my visit history is only available to my account unless I explicitly share it.
Acceptance criteria:
